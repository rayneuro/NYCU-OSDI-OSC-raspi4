...
{   
    ...
    mmio_write(MAILBOX_WRITE , interface);
...
uint64_t mbox_get_VC_base_addr()
{
    ...
    // tags end
    mail_box[7] = TAGS_REQ_END;

    if ( mailbox_call( MBOX_CH_PROP, mail_box ) )
    {
...
void mbox_set_clock_to_PL011() 
{
    ...
    // tags begin
    mail_box[2] = MBOX_TAGS_SET_CLOCK;   // set clock rate
    ...
    mail_box[8] = TAGS_REQ_END;
    
    mailbox_call( MBOX_CH_PROP, mail_box );
}
...
int mbox_framebuffer_init( uint32_t width, uint32_t height, FRAME_BUFFER * fb )
{
    ...
    mail_box[34] = TAGS_REQ_END;

    //this might not return exactly what we asked for, could be
    //the closest supported resolution instead
    if( mailbox_call( MBOX_CH_PROP, mail_box ) && mail_box[20] == 32 &&  mail_box[28] != 0 )
    {
